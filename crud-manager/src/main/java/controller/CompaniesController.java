...
public class CompaniesController extends HttpServlet {
	...
	private void deleteCompany(HttpServletRequest req, HttpServletResponse resp) {
		...
		try {
			...
			if (dao.delete(company)) {
				...
				ControllerUtil.errorMessage(req, "Empresa '" + company.getName() + "' não pode ser deletado. "
	...
	private void insertCompany(HttpServletRequest req, HttpServletResponse resp) {
		...
		try {
			if (dao.save(comp)) {
				ControllerUtil.sucessMessage(req, "Empresa '" + comp.getName() + "' salva com sucesso.");
			} else {
				ControllerUtil.errorMessage(req, "Empresa '" + comp.getName() + "' não pode ser salva.");
			}
		} catch (ModelException e) {
			// log no servidor
			e.printStackTrace();
